# app.py (v4.4-react-multiline — React UI + płynniejszy DnD + multiline karty)
...
BUILD_TAG = "v4.4-react-multiline"
    ...
    def check_references(self):
        ...
        for col in self.columns:
            ...
            for tid in col.task_ids:
                if tid not in task_keys: raise ValueError(f"Task id '{tid}' in column '{col.name}' not found.")
...
def get_board() -> Board:
    if "board" not in st.session_state:
        st.session_state.board = _default_board().model_dump(mode="json")
...
def add_task(column_id: str, t: Task) -> str:
    ...
    save_board(b); bump_rev();  # po dodaniu zmieniamy klucz komponentu
    return tid
...
# ===== Format labelu: wielolinia =====
def item_label_multiline(t: Task) -> str:
    title = t.title.strip()
    desc = t.desc.strip() if t.desc else ""
...
def export_json_button(board: Board):
    ...
    st.download_button("⬇️ Export JSON", json.dumps(data, ensure_ascii=False, indent=2), "board.json", "application/json", use_container_width=True)
...
def import_json_uploader():
    ...
    if up is not None:
        try:
            ...
            save_board(board); bump_rev()
            st.success("Zaimportowano tablicę."); st.rerun()
...
with st.sidebar.expander("Dodaj kolumnę"):
    ...
    if st.button("➕ Dodaj kolumnę", use_container_width=True):
        if new_col_name.strip(): add_column(new_col_name.strip()); st.success("Dodano kolumnę."); st.rerun()
        else: st.error("Podaj nazwę kolumny.")
with st.sidebar.expander("Zmień nazwę kolumny"):
    ...
    if col_opts:
        ...
        if st.button("✏️ Zmień nazwę", use_container_width=True):
            if new_name.strip(): rename_column(col_opts[sel_name], new_name.strip()); st.success("Zmieniono nazwę."); st.rerun()
            else: st.error("Podaj nową nazwę.")
with st.sidebar.expander("Usuń kolumnę"):
    ...
    if col_opts2:
        ...
        if st.button("🗑️ Usuń kolumnę", use_container_width=True, disabled=not confirm):
            move_to = None
            if tgt_name != "—": move_to = dict(others)[tgt_name]
            delete_column(col_opts2[del_name], move_to); st.rerun()

# Dodawanie zadań
st.sidebar.divider()
with elements("add_task_header"): mui.Typography("➕ Dodaj zadanie", variant="h6")
with st.sidebar.form("add_task_form_sidebar", clear_on_submit=True):
    ...
    if sb_submitted:
        if not add_title or not add_title.strip(): st.error("Tytuł jest wymagany.")
        elif not col_map: st.error("Brak kolumn.")
        else:
            ...
            add_task(col_map[add_colname], task); st.success("Dodano zadanie."); st.rerun()
...
with st.sidebar.expander("🛠️ Edycja/Usuwanie zadania"):
    ...
    if task_choices:
        ...
        if c1.button("✏️ Edytuj", use_container_width=True, key="edit_btn"):
            st.session_state.edit_task_id = chosen_tid; st.rerun()
        if c2.button("🗑️ Usuń", use_container_width=True, key="delete_btn"):
            delete_task(chosen_tid); st.success("Usunięto zadanie."); st.rerun()
        cur_done = b.tasks[chosen_tid].done
        if c3.button("✅ Done", use_container_width=True, key="toggle_done_btn"):
            edit_task(chosen_tid, {"done": not cur_done}); st.rerun()
...
if st.session_state.get("edit_task_id"):
    ...
    with st.sidebar.expander("✏️ Edytuj wybrane zadanie", expanded=True):
        with st.form("edit_task_form_sb", clear_on_submit=True):
            ...
            if esub:
                if not etitle.strip(): st.error("Tytuł jest wymagany.")
                else:
                    ...
                    if new_col_id != current_col_id:
                        b2 = get_board()
                        for c in b2.columns:
                            if st.session_state["edit_task_id"] in c.task_ids: c.task_ids.remove(st.session_state["edit_task_id"])
                        for c in b2.columns:
                            if c.id == new_col_id: c.task_ids.append(st.session_state["edit_task_id"])
                        save_board(b2)
                    st.success("Zapisano zmiany."); st.session_state.pop("edit_task_id", None); st.rerun()

# React/MUI Title
with elements("title"): mui.Typography(f"📋 Tablica Kanban — {BUILD_TAG}", variant="h4", gutterBottom=True)

# DnD containers (STABILNY KEY + MULTILINE LABEL)
...
st.caption(f"Build: {BUILD_TAG} — multiline karty + płynny DnD. Import zastępuje stan, Export pobiera snapshot.")
