...
# Wersja: v5.3.5-supabase-modals-overlay (one-modal-at-a-time)
# - Modale: _modal() używa natywnego st.modal jeśli jest; w starszych wersjach Streamlit
#   pokazuje prawdziwy „overlay” na całą stronę z panelem (bez st.dialog).
# - Zawsze tylko jeden modal na raz (kliknięcie jednego zamyka drugi).
...
BUILD_TAG = "v5.3.5-supabase-modals-overlay"
REV_KEY = "_view_rev"
...
def _modal(title: str, key: str | None = None):
    ...
    Nie używamy st.dialog / experimental_dialog (to dekoratory, inne API).
    """
    ...
    class _OverlayPanel:
        def __enter__(self):
            # Pełnoekranowy overlay z panelem – zamykamy w __exit__
            st.markdown(
                f"""
...
<style>
  ...
  /* zmniejsz odstęp pod przyciskami, żeby nie było "białego pola" */
...
# Tylko jeden modal jednocześnie
if open_add:
...
st.caption("Modale z overlayem (jeden naraz). Import zastępuje stan, Export pobiera snapshot. Supabase włączony, jeśli skonfigurowano.")
